...
/**
 * Tests all aspects of the Contract class, including three non-trivial methods
 * 
...
public class SpaceTraderExampleTest extends TestCase {
	...
	public void testContract() {
		...
		if (contract.getDestination() == game.thePlayer.getship()
				.getPlanetName()) {
			Assert.assertTrue(
					"Cannot complete contract when on correct planet",
					contract.canCompleteContract(game));
		} else {
			Assert.assertFalse("Can complete contract when on wrong planet",
					contract.canCompleteContract(game));
		}
		...
		if (type == "goTo") {
			testString = String
					.format("Please deliver this important message to %s."
			...
			testString = String.format(
					"A client on %s is offering %d credits for the safe delivery"
							+ " of %d units of %s.", contract.getDestination(),
					contract.getReward(), contract.getReqAmount(),
					contract.getReqGood());
			Assert.assertEquals(testString, contract.getString());
		} else {
		...
		// example of failure
		/*
		 * if (contract.getDestination() ==
		 * game.thePlayer.getship().getPlanetName()){
		 * Assert.assertFalse("Cannot complete contract when on correct planet",
		 * contract.canCompleteContract(game)); } else {
		 * Assert.assertTrue("Can complete contract when on wrong planet",
		 * contract.canCompleteContract(game)); }
		 */
		...
		testString = String.format(
				"Please deliver this important message to %s."
						+ " You will be paid %d credits upon your arrival.",
				contract.getDestination(), contract.getReward());
		Assert.assertEquals(testString, contract.getString());
		...
		while (game.thePlayer.getship().getPlanetName() == contract
				.getDestination()) {
			game.thePlayer.getship().moveToPlanet(
					game.theMap.getPlanet(game.theMap.planetNames[rand
							.nextInt(7)]));
		}
		Assert.assertFalse(contract.canCompleteContract(game));
		// case: at planet, no goods
		game.thePlayer.getship().moveToPlanet(
				game.theMap.getPlanet(contract.getDestination()));
		Assert.assertFalse(contract.canCompleteContract(game));
		...
		game.thePlayer
				.getship()
				.getInventory()
				.add(game.thePlayer.getship().getInventory()
						.getGood(contract.getReqGood()),
						contract.getReqAmount());
		curMoney = game.thePlayer.getship().getInventory().getMoneyLeft();
		contract.canCompleteContract(game);
		if (game.thePlayer.hasContract) {
			Assert.assertEquals(prevMoney + contract.getReward(), curMoney);
		...
		game.thePlayer
				...
				.add(game.thePlayer.getship().getInventory()
						.getGood(contract.getReqGood()),
						contract.getReqAmount() - 1);
		Assert.assertFalse(contract.canCompleteContract(game));
	}

	public void testAdd() {
		Inventory list = new Inventory();
		Assert.assertEquals(list.goods.size(), 0); // tests successful creation
													// of Inventory object.
		Good good1 = new Good("testGood", 0, 0, 0, 10, 1, 0, true);
		list.add(good1, 1);
		Assert.assertEquals(list.goods.size(), 1); // tests that the the new
													// good is being added after
													// creation of a GoodData
													// object, since it doesn't
													// exist yet.
		list.add(good1, 1);
		Assert.assertEquals(list.goods.size(), 1); // tests that a new GoodData
